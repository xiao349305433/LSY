package com.rokid.remote.record.glutils;
/*
...
public class EGLBase {    // API >= 17
    ...
    public static class EglSurface {
        ...
        EglSurface(final EGLBase egl, final Object surface) {
            ...
            Logger.v(String.format("EglSurface:size(%d,%d)", mWidth, mHeight));
        }
    ...
    private void init(EGLContext shared_context, final boolean with_depth_buffer, final boolean isRecordable) {
        ...
        Logger.d("EGLContext created, client version " + values[0]);
...
//		 Logger.v( "makeCurrent:");
        if (mEglDisplay == null) {
            Logger.d("makeCurrent:eglDisplay not initialized");
        }
        if (surface == null || surface == EGL14.EGL_NO_SURFACE) {
            final int error = EGL14.eglGetError();
            if (error == EGL14.EGL_BAD_NATIVE_WINDOW) {
                Logger.e("makeCurrent:returned EGL_BAD_NATIVE_WINDOW.");
            }
            return false;
        }
        ...
        if (!EGL14.eglMakeCurrent(mEglDisplay, surface, surface, mEglContext)) {
            Logger.w("eglMakeCurrent:" + EGL14.eglGetError());
            return false;
        }
        return true;
    }
...
//		 Logger.v( "createContext:");
    ...
    private EGLSurface createWindowSurface(final Object nativeWindow) {
        Logger.v("createWindowSurface:nativeWindow=" + nativeWindow);
    ...
    private EGLSurface createOffscreenSurface(final int width, final int height) {
        Logger.v("createOffscreenSurface:");
        final int[] surfaceAttribs = {
        ...
        try {
            ...
            Logger.e("createOffscreenSurface", e.getMessage());
    ...
    private void destroyWindowSurface(EGLSurface surface) {
        ...
        Logger.v("destroySurface:finished");
    }
    ...
    private EGLConfig getConfig(final boolean with_depth_buffer, final boolean isRecordable) {
        ...
        if (!EGL14.eglChooseConfig(mEglDisplay, attribList, 0, configs, 0, configs.length, numConfigs, 0)) {
            ...
            Logger.w("unable to find RGBA8888 / " + " EGLConfig");
            return null;
        }
        return configs[0];
    }
