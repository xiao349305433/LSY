...
@Route(path = K.CheckFirst_jizhongqiActivity)
public class CheckFirst_jizhongqiActivity extends BaseBlueToothActivity implements MeterViewBinder.OnZhiShuListener {
    ...
    private int MissClickNum = 0;//漏抄次数
    ...
    protected void onInitAttribute(BaseAttribute ba) {
        ba.mActivityLayoutId = R.layout.m_chip_activity_checkfirst_jizhongqi;
        ba.mTitleText = "质检集中器底板测试";
    }
    ...
    protected void initView() {
        ...
        connectBlueTooth(sensoroDevice);
        initBlueClick();
        checkUpData = getIntent().getParcelableExtra("checkUpData");
        XLog.i("LSYgetInspectionField=====" + checkUpData.getInspectionField());
    }
    ...
    private void setUpdata() {
        ...
        sb.append(checkUpData.getInspectionField());
        sb.append("电池状态:");
        if (Double.valueOf(hashMap.get(MapParams.电池电压)) < 3.5) {
            sb.append("欠压");
        } else {
        ...
        if (accept.getText().toString().equals("64") &&MissClickNum<3) {
            sb.append(";抄表功能（64个表）:正常");
        }else {
            sendMessageToast("请抄64个表");
            return;
        }
