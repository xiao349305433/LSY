...
@Route(path = K.CheckFirstActivity)
public class CheckFirstActivity extends BaseSnBlueToothActivity {
    ...
    private int IsOk = 0;
    private int FirstStepRead = 0;
    private int FirstStepWrite = 0;

    private PrintTool printTool;

    @Override
    protected int onChildLayout() {
        return R.layout.m_chip_activity_checkfirst;
    }
...
/**-------------------------------------------------------------------------------------------------------------------------------------------*/
    ...
    private void setAtDuShu(HashMap<String, String> hashMap) {
        ...
        runOnUiThread(() -> {
            ...
            FirstStepRead = 1;//一键读取已完成
        });

    }


    private void tonext(HashMap<String, String> hashMap) {
        StringBuilder sb = new StringBuilder();
        if (hashMap.get(SAtInstructParams.sAtInstructSendingPower) == null) {
            sendMessageToast(" 请进行第一步一键读取");
            return;
        }
        if (hashMap.get(MapParams.软件版本) == null) {
...
//        sb.append(hashMap.get(MapParams.表电池状态).equals("0") ? "正常" : "欠压");
        ...
        IsOk = FirstStepRead + FirstStepWrite;
        if (IsOk == 2) {
            sb.append(";底板参数配置读取:正常;");
            XLog.i("LSY;底板参数配置读取:正常;");
        } else {
            sb.append(";底板参数配置读取:异常;");
            XLog.i("LSY;底板参数配置读取:异常;");
        }
        ...
        checkUpData.setModuleType(1);
        XLog.i("LSYsb====" + sb.toString());
        checkUpData.setInspectionField(sb.toString());
        checkUpData.setChipNum(sensoroDeviceChoose.sn);
        ARouter.getInstance().build(K.CheckFirst_jizhongqiActivity).withParcelable("sensoroDevice", sensoroDeviceChoose).withParcelable("checkUpData", checkUpData).navigation();
        //     ARouter.getInstance().build(K.CheckFirst_jizhongqiActivity).withParcelable("checkUpData", checkUpData).navigation();
