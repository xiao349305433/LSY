...
@Route(path = K.OnCloudWulianwangduanInitActivity)
public class OnCloudWulianwangduanInitActivity extends BaseBlueToothActivity implements View.OnClickListener {
...
//    private SensoroDeviceSession sensoroDeviceSession;
            ...
            public boolean onLongClick(View v) {
                if (onetoOneConverter.getImagePath() != null) {
                    ...
                    ARouter.getInstance().build(C.FullViewPictureActivity)
                            ...
                            .navigation(OnCloudWulianwangduanInitActivity.this);
                }
                return true;
            }
        });
    ...
    private void setData(OnetoOneConverter onetoOneConverter) {
        if (onetoOneConverter != null) {
            GlideUtil.display(this, onetoOneConverter.getImagePath(), R.drawable.m_twoinit_zx, new SimpleTarget<Bitmap>() {
                @Override
                public void onResourceReady(Bitmap resource, GlideAnimation<? super Bitmap> glideAnimation) {
                    GlideUtil.display(OnCloudWulianwangduanInitActivity.this, ivSelector, onetoOneConverter.getImagePath());
                    onetoOneConverter.setImage(PictureUtil.bitmapToString(resource));
                }
            });
            canSave = true;
     ...
     * 如果args为null则清空旧数据
     */
    private void setFactoryInfo(boolean isFromNet, String... args) {
    ...
    public void onClick(View v) {
        ...
        if (id == R.id.unit) {
            ...
            if (!hasWrite) {
                sendMessageToast("请先设置再读取,保证云端数据与板子数据一致");
                return;
            }
            showDialog = true;
            hasRead = true;
            ...
            if (!canSave) {
                ...
                if (XHStringUtil.isEmpty(onetoOneConverter.getImage(), false)) {
                ...
                LogUtils.i("设备id=" + onetoOneConverter.getUserId());
    ...
    protected void onEventRunEnd(Event event, int code) {
        if (code == MInitTwoCode.GetChangJiaBiaoShiRunner) {
            ...
            if (event.isSuccess()) {
                ...
                onetoOneConverter = (OnetoOneConverter) event.getReturnParamAtIndex(1);
                if (codeReturn == 0) {
                    setData(onetoOneConverter);
            ...
            if (event.isSuccess()) {
                ...
                switch (codeReturn) {
                    ...
                    case -1:
                        ...
                        break;
                    case 0:
                        sendMessageToast("上传成功");
                        EventBus.getDefault().post(new RefreshOnCloudEvent());
                        break;
                    case 1:
                        sendMessageToast("参数异常");
                        break;
                    case 2:
                        sendMessageToast("权限限制");
                        break;
                }
            }
        }
                        ...
                        if (isUnit) {
                            ...
                            if (softVersion >= 4) {
                                ...
                                if (!XHStringUtil.isEmpty(hashMap.get(MapParams.发送功率), false)) {
                                    sb.append(hashMap.get(MapParams.发送功率));
                                    sb.append("dbm");
                                }
                            }
                            sb.append("\n备注: ");
