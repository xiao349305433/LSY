...
public class LoginRunnerPhone extends CommonRunner {
    ...
    public void onEventRun(Event event) {
        ...
        params.put("code", pwd);
        Request<String> request = NoHttp.createStringRequest(URLUtils.getIP() + URLUtils.FSLoginURLPHONE, RequestMethod.POST);
        request.add(params);
        Response<String> response = requestString(request);
        if (response.isSucceed()) {
            String result = response.get();
            Gson gson = new Gson();
            FactoryLoginData aLoginAnalysis = gson.fromJson(result, FactoryLoginData.class);
            event.setSuccess(true);
            event.addReturnParam(aLoginAnalysis);
        }
    }
}
